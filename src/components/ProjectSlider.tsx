...
export function ProjectSlider() {
  ...
  const move = (direction: 'next' | 'previous') => {
    ...
    if (element)
      element.scrollBy({
        left: (direction === 'next' ? 1 : -1) * element.clientWidth * 0.78,
        behavior: 'smooth',
      });
  };
  const onPointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    slider.current.classList.add('is-dragging');
  };
  const onPointerMove = (event: PointerEvent<HTMLDivElement>) => {
    if (drag.current.active && slider.current)
  ...
  const endDrag = (event: PointerEvent<HTMLDivElement>) => {
    ...
    slider.current.classList.remove('is-dragging');
    if (slider.current.hasPointerCapture(event.pointerId))
        ...
        {projects.map((project) => (
          <article className="project-card" key={project.title}>
            <div
              ...
              aria-label={`Placeholder visual ${project.image}`}
            >
